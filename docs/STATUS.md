> 更新时间：2026-09-05（MIT-460 P7-names：**标记函数真名解析落地**（无派单直接开发；同日 MIT-456/457/458/459 已合入）：SDK `WVMP_BEGIN(fn)` 把 `#fn` 传给 `marker_begin(const char*)`（桩体不变、名字不落指令，仅令编译器在 begin 调用点前物化字符串地址：x64 = lea rcx,[rip+disp] 紧邻 E8 / x86 = push imm32 紧邻 E8；物化在区域外不进虚拟化管道）；scan_core 新增 locate_name_operand 纯函数定位物化（无匹配 = nullopt 回退）；marker_scan_pass 解析可打印 C 字符串（≤64B）写 `FunctionRegion.name`，失败/旧产物回退地址名 + Note。**首通修正**：rip 基 = E8 起点（call_next−5）非区域起点（真名差 5 字节漂移实录：compute_beta → te_beta）；35 个样本 asm 的硬编码修饰名 `?marker_begin@…YAXXZ` → `…YAXPEBD@Z` 批量更新。**验证**：ctest **19/19**（scan_core 定位器 ×3 新增 + 集成测试翻正：sample 名断言 compute_alpha/beta、合成/x86 ml 产物回退 Note 计数）；E2E：sample 管道区域清单现真名 `[0] compute_alpha rva=0x1016`，打包双跑 byte-identical；wvmpTest（自含无名桩）14 stubs 回退名零回踩。CLI 区域清单行升级为 `[i] <名> rva=0x…`（T3 name= 选择器 UX 落点））
