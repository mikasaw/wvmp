> 更新时间：2026-09-05（MIT-458 crypt-v1：**M3 工作流 1 字节码加密收口**（无派单直接开发；同日 MIT-456 push-mem、MIT-457 config-v1 均已合入 main）：xor_chain codec（`vm/regvm/codecs/` 填实，create_codec 工厂；u32 字 LE、state*0x9E3779B1+c 链吃密文、加密=解密同递推异面）+ CryptPass（Transform，virtualize 之后；独立 Rng seed^盐 派生 key0，不扰动 ctx.rng——无 crypt 管道随机序列逐字节不变）+ stub 入口 one-shot 解密块（双 arch 织入于序言前，6 push/pop 保存被蹭 GP 全集；尾旗标 8B flag=1→0；密钥立即数嵌入=每目标嵌入）+ 32B 头明文保留（stub_link x86 白名单 gate 与 read_blob 消费头不受扰）+ plan 经 kCryptPlan 扩展槽传递（stub_link 无 plan = 逐字节 v1 前现形）。**验证**：ctest 18/18（新增 regvm_codec_tests ×5 + crypt_tests ×3）；wvmpTest 双 arch crypt 管道（7 passes）打包双跑 103/103 与原生 byte-identical（x64 14 stubs / x86 12 stubs，seed 12345/1/99999 三种子）；无 crypt 基线 14 stubs 双跑 diff-0 零回踩；x86 电池全绿。**D1 边界**：首入口并发双重解密未防护；明文首执行后驻留内存；每函数 crypt 档位待 ProtectLevel 追加）
