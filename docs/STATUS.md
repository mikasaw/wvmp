...
> 更新时间：2026-09-02（MIT-454 X6：**x86 残 gate 双面包翻正**——B 族 push-imm x86 开面（#33 定位修正：卡点在 translator 非 lifter；x86 Imm 形单 op 发射，x64 维持 gate 留 X6b）+ A 族 X3d SSE 32 op 批迁（x64 表逐 op 32 位镜像 + stub xmm 同步 + ucomis flags 面；批迁当场修 build_xmm_store_x86 地址/src 临时同用缺陷）；wvmpTest x86 残面 13→**3** / stubs 9→**11** / 103-kernel 真虚拟化率 64.3%→**78.6%**；样本池 14→**15**（pushimm）+ wvmp_x86_sse_sample 翻案 gate→真虚拟化 byte-exact；multiseed 320→**325**；x86 电池 37→**43**；x86 handler 表 60→**92**；x64 dump `ffd47289…` 三批复验逐字节恒等）
