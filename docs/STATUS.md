> 更新时间：2026-09-05（MIT-457 config-v1：**M3 配置系统首版——每函数保护档位**（无派单直接开发；同日 MIT-456 push-mem 已 ff 合入 main `2c83eba`）：TOML 新增 `default_level`（none/virtualize，可省略=virtualize）+ `[[functions]]` 每函数规则（选择器 `rva`（begin 标记 RVA，TOML 0x 字面量）/`index`（扫描序）恰一 + `level` 恰一）；解析期拒绝：未知档位/双选择器/无选择器/负值/同选择器重复；模型 `ProtectRules`/`ProtectLevel`（append-only enum，pitfall #34 纪律）落 `framework/protect_levels.hpp`，CLI 经扩展槽 `kProtectRules` 下发，virtualize 消费（`level=none` → 函数保持原生 + Note）；CLI 新增 stdout `区域清单` 单行（rva/index 选择器口径来源）与 `档位` 摘要行；槽缺席 = 直连 API 行为与 v1 前逐字节一致。**真靶标验证**：wvmpTest x64 两条 `level=none` 规则 → stubs 14→12、双跑 103/103 与原生 byte-identical；无规则配置 → 14 stubs 基线不变。ctest 16/16（新增 config 解析 ×8 + virtualize 选择 ×4 用例）。选择器 `name=`（真名）依赖 P7-names（SDK 标记宏现弃名），为下一排队项；本单为 M3 工作流 1-5（crypt/mutate/anti_debug 等）的每函数档位消费面奠基）
