> 更新时间：2026-09-05（MIT-463 anti_debug-v1：**M3 工作流 3 反调试首版——PEB 检查 + FailFast**（无派单直接开发；同日 MIT-456~462 均已合入）：AntiDebugPass（Transform）产 AntiDebugPlan（技术位 + 响应）写 kAntiDebugPlan 槽（INTERFACE 契约头，stub_link 零对象耦合）；stub 入口织入 PEB 检查块（双 arch：x64 PEB=[gs:0x30]+0x60、NtGlobalFlag@+0xBC / x86 PEB=[fs:0x18]+0x30、@+0x68；BeingDebugged byte[+2] + NtGlobalFlag&0x70 双检查；命中 = FailFast 写 [0] 确定性崩溃；被蹭 acc push/pop 恢复）。**验证**：ctest **20/20**（新增 anti_debug_tests ×3）；wvmpTest 双 arch 9-pass 全栈（mutate+crypt+anti_debug）双跑 103/103 与原生 byte-identical（x64 14 stubs / x86 12 stubs）；字节级判别：NtGlobalFlag 读（mov eax,[rax+0xBC] / mov eax,[eax+0x68] disp8）在 adb 产物恰每 stub 一份、无 adb 基线 0 份。**D1 边界**：v1 检查面 = PEB 双检查（rdtsc/DRx/NtQuery 留后续）；响应 = FailFast（与天然崩溃不可区分；ExitProcess 需 import 解析，后续）；检查仅覆盖入口 stub（C1 gate 函数原生无检查）；每函数粒度随 T3 后续）
