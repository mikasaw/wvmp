> 更新时间：2026-09-05（MIT-459 mutate-v1：**M3 工作流 2 代码变异首版——Nop 填充落地 + junk-Mov 宁 gate 勿错挂账**（无派单直接开发；同日 MIT-456/457/458 均已合入）：MutatePass（Transform，lifter 后 virtualize 前，消费 ctx.functions 内联 IR）确定性两族变异——Nop 填充（P=0.10，零寄存器/零 flags 面）+ 死寄存器垃圾 Mov（块内向后 liveness，含 callgate 隐式读修正：x64 Call 处 rcx/rdx/r8/r9/rax 为原生参数面必活）。**junk-Mov 收口裁定 = 默认关闭（kEnableJunkMov=false）**：修复 callgate 读集后 wvmpTest 全栈 E2E 仍有未定位槽消费面（同地址原生代码执行发散实录，详见 GAPS mutate 节），按宁 gate 勿错纪律 v1 只发 Nop（E2E byte-exact 已证）。**验证**：ctest **19/19**（新增 mutate_tests ×5：确定性/异 seed 差异/插入面不变量+独立 liveness 复算/档位跳过/空块）；wvmpTest 真靶标 mutate+crypt 全栈（8 passes）双跑 103/103 与原生 byte-identical（14 stubs，105 Nop 注入，seed 12345/777）；无 mutate 基线零回踩。独立 Rng（seed^盐）不消费 ctx.rng——无 mutate 管道随机序列逐字节不变）
