> 更新时间：2026-09-05（MIT-461 config-v2：**配置系统第二版——name 选择器 + 每函数 crypt 覆写**（无派单直接开发；同日 MIT-456~459 均已合入）：[[functions]] 选择器扩至 rva/index/name 三选一（name 依赖 MIT-460 真名解析，跨重编译稳定）；FunctionProtectRule 增 crypt 三态覆写（缺省=跟随管道全局、false=豁免、true=强制；仅 crypt pass 在管道时有意义）；ProtectRules::crypt_for 仲裁序同 level_for（index→rva→name 后评胜，仅统计显式覆写）；crypt pass 消费覆写——豁免函数不进 plan；密钥流按 vfs 序无条件消费（含豁免），加豁免规则不改任何已加密函数 key0（验收 REJECT 项修复 + 单测钉）。**验证**：ctest **19/19**（新增 config ×4 + crypt 豁免 ×1）；E2E：sample 真名管道 `name="compute_alpha" crypt=false` → 已加密 1/2 + 豁免 Note + 打包双跑 byte-identical；严格 schema 同步放行 crypt/name 键、拒未知键）
