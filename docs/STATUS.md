...
### MIT-494p (T38 · Defender 误报量化) ✅ 2026-09-12
- MpCmdRun -DisableRemediation 受控矩阵（seed×管道×文件名×路径，含与
  被检出字节完全相同的重打包产物）：高水位假设否证、文件名/seed 变量
  排除。验收补全 6 条检出历史（ThreatID 251873 = Program:Win32/Contebrew.
  A!ml 云 ML 判定）→ 修正图景：**云端 ML 判定按文件哈希分别持久化、翻
  转不同步**（S 族 ≤80min 转阴 / T37 族 ≥24min 未翻转，签名全程未变）。
  误报与布局参数无稳定关联，不可参数化缓解；应对 = 申诉 + 语义验证解
  耦 + 哈希级时效性标注。MIT-494n 高水位推测同步否证修正。证据链 GAPS
  MIT-494p。
### MIT-494q (T40 · 同词密度双 arch 微基准) ✅ 2026-09-12
- wvmp_hotloop_asm_sample（x64 asm 区域，与 x86 asm 样本同 13 条指令循
  环体）+ measure_hotloop.sh ARCH=x64asm。词流实测对账：x64 13 词/迭代
  vs x86 16 词（差 = x86 iters 槽装载链 3 词 + cmp mem 展开）。**每词
  dispatch 成本：x64 1.653ns vs x86 1.926ns = 1.17×**（T35 的 1.58× 系
  词密度混杂放大）——x86 dispatch 为解释器优化首要靶标（自身基线 ~14%
  收益空间）。开发实录：x64 asm 栈对齐 sub rsp,28h
  先例复用。ctest 23/23 不变；证据链 GAPS MIT-494q。

### MIT-494r (T41 · 解释器词形态 × 每词成本侦察单) ✅ 2026-09-12
- 词形态矩阵 7 测量点（4 新样本：LD/ST 密度、分支/标志密度 × 双 arch +
  既有纯 ALU/C 自然）：**慢词族定案 分支/标志(2.051/1.802ns) > 纯 ALU
  (1.920/1.673) > LD/ST (1.683/1.438) > C 自然(1.327)，双侧一致**；
  x86 税均匀 1.14-1.17×（三形态稳定，与 T40 闭合）。静态对读（runtime
  dump 通道）：dispatch x86 10 指令/7 访存 vs x64 7/3（pc 内存常驻 +
  双 dword 取指 + 双帧溢写）；**Jcc 147/149 指令 = 热词流中唯一大体量
  handler**（load/store/mov 的 ~3 倍；cmovcc 576 等更大者均为冷面）
  （15 路条件链平均 ~15 指令/次）。T42 立案：①cond 链跳表化 ②x86 pc_
  寄存器常驻 ③x86 dispatch 惰性 aux——合并上限 x86 ≈x64 平价、整体
...
- +3 VmOp（kVmOpMax 146）：YmmMov/YmmLoad/YmmStore —— vmov* ymm 32B 传送折叠（v24..31 槽复用 = ymm0..7，VmOp 域消歧）；**按需 stub ymm 同步变体**（词流扫描含 Ymm* 词才启用，无 ymm 产物字节恒等，AVX 机器要求收窄到 ymm 类区域）；**混排 gate**（Ymm* + legacy SSE 同函数 → fail-closed 原生，wave2③ 前置保守面）；y mm8..15/x86 gate 继承。硬件真值 YmmDataPathHardwareTruth（32B 面/内存逐位）+ YmmSyncVariantBytes + 六编码 kstool 钉板；426/428 位宽闸负例翻正（VexYmmMovapsNowFolded 等）；dump 门扩 YMM_HANDLERS；样本 wvmp_ymm_data_sample（③正例 + 混排负例）。验证：ctest 23/23、全池 365/365 REQUIRE_REAL、dump 门 ymm 三 handler PASS（101 handlers）、单 pack stub 3 全 ymm 变体 + 混排 note 命中。
  证据链 GAPS MIT-512。