...
1. **高水位假设否证**——87.9% 高水位矩阵 6/6 无威胁（含与被检出字节
   完全相同的重打包产物）；
2. 文件名/路径/seed 变量全部排除（packedT38 复刻检出原名原路径亦无威
   胁）；
3. 唯一与检出相关的变量 = **提交时间窗**（23:10–00:14 检出，00:30 起
   同字节各形态全部转阴）→ Defender 云端 ML 分类器时间窗瞬时误报，判
   定后撤（本地签名版本同期未变 = 非签名更新）；
4. **定性修正**：avscan 误报 = 偶发性云引擎误报，与产物布局参数无稳定
   关联——**不可通过调产品参数系统性缓解**；工程应对 = ① 误报申诉（
   Microsoft 安全情报提交）② E2E 语义验证与杀软判定解耦（打包后立即
   运行抢清除延迟，或扫描面加排除）③ avscan PASS/FAIL 结论标注时间窗
   时效性。

**工具遗产**：MpCmdRun -DisableRemediation 非干扰扫描口径（可编入门
户脚本，替代被动等 Get-MpThreatDetection）。
