...
// 密文流之后的 8B 尾旗标：[u32 flag][u32 reserved]。
// flag 初值 1 = 已加密待解密；stub 入口解密块 one-shot 语义（xor_chain
// 同一段内存只可执行一次，重复执行 = 再加密）；解密后 stub 写 0。
...
struct CryptedFunction {
    ...
    std::vector<u8> encrypted_blob;  // 32B 明文头 + 密文流 + 8B 尾旗标
    u64 stream_bytes = 0;            // 流长（= 明文流长；加密定长）；旗标位于流起点 + stream_bytes
};
