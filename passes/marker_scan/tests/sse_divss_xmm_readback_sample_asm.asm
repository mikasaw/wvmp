; MIT-374: SSE 浮点除 divss 影子样本的 MASM helper —
; sse_divss_xmm_readback_sample (与 wvmp_sse_div_sample 的 sse_divss 同 IR 配对,
; 沿用 MIT-389 sse_subss_xmm_readback_sample_asm.asm 模式)。
;
...
;   1) xr_divss_s01: 区域 divss xmm0,xmm1 —— 与主样本 sse_divss 同 IR。
;      返回值 = HALT 时 ctx.xmm[0] 经 stub 恢复路径的唯一出口, 即 ctx 读回。
;   2) xr_divss_s23: 区域 divss xmm2,xmm3 —— 不同槽位探针 (reg=26/27),
...
;   3) xr_divss_keep: 区域 divss xmm0,xmm1, 区域后把 xmm0..xmm3 全部 128-bit
;      store 到缓冲 —— main 打印全部 4 槽 × 4 lane, 逐位比对 (未触碰槽位
;      必须保持 VM 入口同步值)。
;
; MSVC x64 不支持 inline asm; 区域只含白名单 SSE REG-REG (F3 0F 5E mod=11)。
; load / store / movaps 全部在区域外 (与 sse_div_sample_asm.asm 模式一致)。
;
...
; float xr_divss_s01(float a, float b):
;   区域 = divss xmm0, xmm1 (与 wvmp_sse_div_sample 的 sse_divss 同 IR)。
;   返回值 = a/b。虚拟化路径: stub 入口同步 xmm0..xmm7 → ctx.xmm[0..7],
;   Divss handler 更新 ctx.xmm[0], HALT 后 stub 恢复 xmm0..xmm7, xmm0 即
...
; float xr_divss_s23(float a, float b, float* out_dst, float* out_src):
;   Win64 ABI: a=xmm0, b=xmm1, out_dst=r8, out_src=r9。
;   区域外: xmm2=a, xmm3=b (movaps); 区域 = divss xmm2, xmm3;
...
xr_divss_s23 PROC
    ...
    ; ===== marker region begin =====
    call ?marker_begin@sdk@wvmp@@YAXXZ
    divss xmm2, xmm3                 ; 真 divss REG-REG (F3 0F 5E D3, mod=11)
    nop
    nop
    nop
    nop
...
; float xr_divss_keep(float a, float b, void* slots):
;   Win64 ABI 按参数**位置**分配寄存器: a=位置1→xmm0, b=位置2→xmm1,
;   slots=位置3→r8 (整型槽 rcx/rdx/r8/r9 与浮点槽 xmm0-xmm3 各自独立按位分配)。
;   区域 = divss xmm0, xmm1; 区域外把 xmm0..xmm3 全部 128-bit store 到 slots
