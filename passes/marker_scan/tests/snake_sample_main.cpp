...
//
// update_food(): 重生食物坐标 (提取为内联函数, 让 lifter 识别 inline 边界)
...
//   - 关键: 提取为内联函数后, tick() 内的 `if (ate != 0)` 跨 update_food
//     调用不再是跨区域控制流, 因为函数调用天然跨 WVMP_BEGIN/END 区域边界
//     (call gate, MIT-249 已支持整数 ABI)。
static inline void update_food(int s) {
...
__declspec(noinline) static void tick(int new_dir) {
    ...
    // 7) ate 时: 分数+1, 重生食物 (调 update_food 内联函数, lifter 识别 inline 边界)
    if (ate != 0) {
        g_score = g_score + 1;
        update_food(s);
    }
