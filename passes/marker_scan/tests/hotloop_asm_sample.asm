...
;
; Loop body = the SAME 15 dword instructions as the x86 asm sample (mov/
; xor/shl/shr/add/inc/cmp/jb on 32-bit subregisters), so both dispatchers
; walk an equal word count per iteration -- per-word cost becomes directly
; comparable (measure_hotloop.sh, ARCH=x64).
;
...
; rides r8d (volatile), args consumed before the marker_begin call.
;
