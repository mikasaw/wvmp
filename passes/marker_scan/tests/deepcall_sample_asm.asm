...
;     合计每层              0x60
;   递归 16 层 (levels=0x10): 总树深 = 16 x 0x60 = 0x600 >= 0x400 (派活单 §B.6)
;   旧预算 0x1D8 (native_sp-0x28 到 ctx 顶 native_sp-0x208) << 0x600
;   → 修复前该样本必砸穿 VmContext (反证, 验收 #8); 修复后树在 host_rsp
;   下方 0x1000 专用窗口内生长 (最深 ns-0x628 > 窗口底 ns-0x1028, 余量
;   0xA00), 与 ctx / stub 保存区零重叠。
;
