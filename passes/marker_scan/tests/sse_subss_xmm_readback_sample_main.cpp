// MIT-389: WVmpVerifier 验收门 A.2 影子样本——SSE 浮点减 subss 的
// ctx.xmm 运行时读回断言 (与 wvmp_sse_sub_sample 的 sse_subss 同 IR 配对)。
//
...
//
//   1) s01: subss xmm0,xmm1 (与主样本同 IR) → 返回值 = ctx.xmm[0] 读回;
//   2) s23: subss xmm2,xmm3 (不同槽位探针, reg=26/27) → 返回值 + dst/src
//      槽位状态 (src 槽必须等于输入 b, handler 不得越槽写);
//   3) keep: subss xmm0,xmm1 后 xmm0..xmm3 全量 128-bit 快照 → 4 槽 × 4 lane
//      全部打印, 未触碰槽位必须保持入口同步值。
//
...
extern "C" float xr_subss_s01(float a, float b);
extern "C" float xr_subss_s23(float a, float b, float* out_dst, float* out_src);
extern "C" float xr_subss_keep(float a, float b, void* slots);

int main() {
    // 1) 同 IR 读回: subss xmm0,xmm1 → 5.5f - 2.0f = 3.5f
    const float s01 = xr_subss_s01(5.5f, 2.0f);
    std::printf("[ctx_xmm0_s01]=%a\n", static_cast<double>(s01));

    // 2) 不同槽位探针: subss xmm2,xmm3 → 7.25f - 3.5f = 3.75f
    ...
    const float s23 = xr_subss_s23(7.25f, 3.5f, dst4, src4);
    ...
    // 3) 槽位保持快照: subss xmm0,xmm1 → 9.75f - 4.25f = 5.5f;
    //    xmm1..xmm3 = 未触碰槽位 (VM 入口同步值), 4 槽 × 4 lane 全打。
    alignas(16) unsigned char slots[64] = {};
    const float sk = xr_subss_keep(9.75f, 4.25f, slots);
